...
#if !defined(SEQUENTIAL)
int parallelSearch (int *totalExplored, int *move, int best, int *l, int noOfMoves, BITSET64 c, BITSET64 u, int noPlies, int o, int minscore, int maxscore) //*l is list of moves
{
    int pid = fork();
    if (pid == 0)
    {
        ...
        {
                multiprocessor_wait(processorAvailable);     // wait for a processor to become available so we only spawn children when the hardware is available
                   
                if (fork() == 0);
                {
                            //alphaBeta(); search //best move using alphabeta
                            alphaBeta (l[i], c, u, noPlies, o, minscore, maxscore);
                            mailbox_send(barrier, &move_score, &move_index, &positions_explored);
                            multiprocessor_signal(processorAvailable); //signals the core is available
                            exit(0);
